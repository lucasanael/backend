...
const movimentoController = require('../models/movimentoController');

exports.listarTodos = (req, res) => {
movimentoController.findAll((err, clientes) => {
if (err) {
res.status(500).send({ message: 'Erro ao buscar movimento'
});
} else {
res.send(clientes);
}
...
if (err) {res.status(500).send({ message: 'Erro ao buscar movimento'
});
} else if (!cliente) {
res.status(404).send({ message: 'Movimento não encontrado'
});
} else {
res.send(cliente); 
...
res.status(500).send({ message: 'Erro ao atualizar movimento' });
} else {res.status(200).send({ message: 'Movimento atualizado com sucesso' });
    }
    });
    };
    ...
    if (err) {
    res.status(500).send({ message: 'Erro ao excluir movimento'
    });
    } else {
